...
option reslim=50000;            // timelimit for solver
...
positive variables
                 ...
                 //s_sqr_up[gens,t]
                 //s_sqr_dn[gens,t]
                 i_sqr(n,n,t)    'squared current magnitude'
...
def_flow1(n,j,t)$(conec(n,j) = 1) ..     sqr(p_flow[n,j,t]) + sqr(q_flow[n,j,t]) =l= (i_sqr(n,j,t)*voltsq(n,t));
def_flow2(n,j,t)$(conec(n,j) = 1) ..     p_flow[n,j,t] + p_flow[j,n,t] =e= res(n,j)*i_sqr(n,j,t);
def_flow3(n,j,t)$(conec(n,j) = 1) ..     q_flow[n,j,t] + q_flow[j,n,t] =e= reac(n,j)*i_sqr(n,j,t);

def_flow4(n,j,t)$(conec(n,j) = 0) ..     p_flow[n,j,t] =e= 0;
def_flow5(n,j,t)$(conec(n,j) = 0) ..     q_flow[n,j,t] =e= 0;
def_flow6(n,j,t)$(conec(n,j) = 0) ..     i_sqr[n,j,t] =e= 0;
...
def_voltage(n,j,t)$(conec(n,j) = 1 and ord(n) > ord(j)) ..   voltsq(j,t) =e= voltsq(n,t) - 2*(res[n,j]*p_flow[n,j,t] + reac[n,j]*q_flow[n,j,t]) + [sqr(res[n,j]) + sqr(reac[n,j]) ]*i_sqr(n,j,t) ;
...
//                                 pf_lin(n,j,t)*res[n,j]+qf_lin[n,j,t]*reac[n,j] =l= 0.001;
...
parameter SOCP_tightness(n,j,t);
SOCP_tightness(n,j,t) = (sqr(p_flow.l[n,j,t]) + sqr(q_flow.l[n,j,t])) - (i_sqr.l(n,j,t)*voltsq.l(n,t));
//sum_cons = sum((t), losses.l(t)) + sum((n,t), dc(t,n));
//all_gen_sum = sum((i,t), x.l(i,t)) + sum(t, Ps.l(t));
display z.l, p_flow.l, q_flow.l, i_sqr.l, voltages, y_up.l, y_dn.l, pup.l, pdn.l, rup.l, rdn.l, v.l, sup.l, squp.l,
flow_limit, p_inj.l, q_inj.l, def_flow1.l, def_flow2.l, solver_time.l,
loss_p, loss_q, pf_lin.l, qf_lin.l, check_suff_cond, SOCP_tightness;   //suff_cond.l, suff_cond.m,

//display power_balance.m, transmission_constr2.m, transmission_constr1.m;

execute_unload "congestion_management_SOCP_%gdxFileName%.gdx" z.L v.L rup.L rdn.L pdn.L pup.L r_fin drl LMP p_TSO_fin p_fin reg_fin input_data A P_DR_resp P_DR_reb Tresp Treb voltages p_flow q_flow q_inj final_dispatch p_inj q_inj sup.l squp.l i_sqr.l solver_time.l SOCP_tightness;
...
execute 'gdxxrw.exe congestion_management_SOCP.gdx par=reg_fin rng=sheet1!A1'
execute 'gdxxrw.exe congestion_management_SOCP.gdx par=input_data rng=sheet1!A20'
execute 'gdxxrw.exe congestion_management_SOCP.gdx par=voltages rng=sheet1!A40'
execute 'gdxxrw.exe congestion_management_SOCP.gdx var=p_flow rng=sheet1!A80'
execute 'gdxxrw.exe congestion_management_SOCP.gdx var=q_flow rng=sheet1!A160'
execute 'gdxxrw.exe congestion_management_SOCP.gdx var=q_inj rng=sheet1!A240'
$offtext
