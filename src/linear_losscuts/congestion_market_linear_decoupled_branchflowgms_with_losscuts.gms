$ontext
This file runs the congestion management with asymmetric block offers, using a linear model for the OPF, with approximation of losses using cuts. The model is run iteratively.
$offtext

$eolcom //
option iterlim=999999999;     // avoid limit on iterations
option reslim=3000;            // timelimit for solver
option optcr=0.00;             // gap tolerance
Option optca = 0
...
positive variables
                 ...
                 loss[n,t]  'active losses assigned to node n'
                 loss_q[n,t]     'reactive power losses assigned to node n'
                 ex_power[n,t]   'slack variable y'
...
objective ..
         ...
         - sum([n,t], 0.00000001*ex_power[n,t]);
...
inj_real(n,t) ..                         p_inj(n,t) - loss(n,t) - ex_power(n,t) =e= sum(j, p_flow(n,j,t) );
...
inj_imag2(n,t) ..                        q_inj(n,t) =e= sum(ns, ( y_up[ns,t] - y_dn[ns,t] ) * loc_ns(ns,n) )
                                                         + sum(i, ( y_up[i,t] - y_dn[i,t] )*loc_i(n,i)) - dcq(t,n)
                                                         + sum(j, B_nodal[n,j]/2) * voltsq[n,t];
...
power_balance1(t) ..                     sum(n, p_inj(n,t) - loss(n,t) - ex_power(n,t)) =e= 0;
...
r_decl(n,t) .. p_inj(n,t) =e= sum([d,c],(rup[d,c,t] - rdn[d,c,t]) * loc_c(c,n)) - sum(c, drl[t,c] * loc_c[c,n])
                         ...
                         - dc[t,n] + sup[t,n] - sum(j,G_nodal[n,j]/2)*voltsq[n,t] ;


def_losses(loc,n,t)$(cut_active(loc) = 1) .. loss[n,t] - sum([j], res[n,j]*flow_fix(loc,n,j,t)*p_flow[n,j,t]  ) =g= -loss_fix(loc,n,t);
def_qlosses(loc,n,t)$(cut_active(loc) = 1) .. loss_q[n,t] - sum([j], reac[n,j]*flow_fix_q(loc,n,j,t)*q_flow[n,j,t]  ) =g= -loss_fix_q(loc,n,t);
...
file data /logfile_losscuts_%gdxFileName%.log/;
put data;

parameter epsilon_tol,low_bound, up_bound;
epsilon_tol = 0.005;
up_bound=+INF;
low_bound=-INF;
//dlmp.optFile = 1;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
loop(lc$( up_bound - low_bound > epsilon_tol),

solve dlmp using mip minimizing z;
//abort$(dlmp.modelstat>=4) "Problem not solved to optimality";
//display z.l;
voltages(n,t) = sqrt(voltsq.l(n,t));

loss_fix(lc,n,t) = sum(j, res(n,j)*sqr(p_flow.l[n,j,t] ) )/2;
loss_fix_q(lc,n,t) = sum(j, reac(n,j)*sqr(q_flow.l[n,j,t] ) )/2;
cut_active(lc) = 1;
flow_fix(lc,n,j,t) = p_flow.l[n,j,t];
flow_fix_q(lc,n,j,t) = q_flow.l[n,j,t];

up_bound = sum([n,t],loss_fix(lc,n,t));
low_bound = sum([n,t], loss.l[n,t] );

solver_time.l(lc) = dlmp.resusd;

if(ord(lc) = 1,
put 'Cut Number ':>4:0, ' ', 'Objective':>12:2, ' ', 'Fix_loss':>12:2, ' ', 'Up Bound':>12:2, ' ', 'Low Bound':>12:2, ' ',  'Cut':>4:0, ' ', 'Loss':>5:3, ' ', 'Time':>12:3 /;
);
put lc.tl:>4:0, ' ', z.L:>12:2, ' ', loss_fix(lc,'n1','t1'):>12:2, ' ', up_bound:>12:2, ' ', low_bound:>12:2, ' ',  sum(loc,cut_active(loc)):>4:0, ' ', loss.l('n1','t1'):>5:3, ' ', solver_time.l(lc):>12:3 /;


//pflowsq(n,j,t) = sqr(p_flow.l[n,j,t]);

display z.l, pup.l, p_flow.l, q_flow.l, voltsq.l, voltages, y_up.l, y_dn.l, loss.l, p_inj.l, q_inj.l,
def_losses.l, loss_fix, flow_fix, loss_fix_q, flow_fix_q, ex_power.l;
)
;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
parameter LMP(n,t), total_time
;

total_time = sum(lc,solver_time.l(lc));
...
display z.l, p_flow.l, q_flow.l, voltages, v.l, y_up.l, y_dn.l, pup.l, pdn.l, rup.l, rdn.l, v.l, sup.l, squp.l,
flow_limit1.l, p_inj.l, q_inj.l,total_time, ex_power.l;// constant_powerfactor.l, constant_powerfactor.m;

//display power_balance.m, transmission_constr2.m, transmission_constr1.m;

execute_unload "congestion_management_linear_decoupled_losscuts_%gdxFileName%.gdx" z.L rup.L rdn.L pdn.L pup.L v.l r_fin dc LMP p_TSO_fin p_fin reg_fin input_data A P_DR_resp P_DR_reb Tresp Treb voltages p_flow q_flow p_inj q_inj;

execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx par=reg_fin rng=sheet1!A1'
execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx par=input_data rng=sheet1!A10'
execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx par=voltages rng=sheet1!A25'
execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx var=p_flow rng=sheet1!A35'
execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx var=q_flow rng=sheet1!A45'
execute 'gdxxrw.exe congestion_management_linear_decoupled_losscuts.gdx var=q_inj rng=sheet1!A55'
